    ...
    React.useCallback(() => {
      ...
      fetch(
        ...
        .then(result => {
          ...
          console.log("response from orders =====",data?.orders );
          if (data?.status === true) {
        ...
        renderItem={data => {
          return (
            <View
              ...
              <View style={{height:40,width:"100%",alignSelf:"center",backgroundColor:Theme.colors.primaryColor,alignItems:"center",justifyContent:"center",borderRadius:5,marginTop:10,flexDirection:"row"}}>
                <Text style={{color:"white",fontSize:14,fontFamily:Theme.fontFamily.semibold,marginRight:20}}>Compare</Text>
                <Icon name='pluscircle' type='antdesign' color={'white'} size={15}/>
              </View>
            </View>
          );
        }}
